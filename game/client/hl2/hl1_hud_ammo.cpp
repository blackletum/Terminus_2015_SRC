...
private:
	...
	CHudTexture		*ammo_label;
	CHudTexture		*progress_bar;
	float	m_flFade;
};
...
void CHudAmmo::Paint( void )
{
	...
	if (!ammo_label)
	{
		ammo_label = gHUD.GetIcon("ammo_label");
	}
	if (!progress_bar)
	{
		progress_bar = gHUD.GetIcon("progress_bar");
	}

	if (!ammo_label || !progress_bar)
	{
		return;
	}
	...
	if ( pActiveWeapon->GetPrimaryAmmoType() != -1 )
	{
		...
		if ( pActiveWeapon->UsesClipsForAmmo1() )
		{
			...
			x = DrawHudNumber( x, y, pActiveWeapon->Clip1(), clrAmmo, false );
...
//			vgui::surface()->DrawFilledRect( x, y, x + nBarWidth, y + nFontHeight );
			progress_bar->DrawSelf(x, y, clrAmmo);

			x += nBarWidth + nFontWidth / 2 + 3;
		...
		else
		{
			...
			x = DrawHudNumber( x, y, pPlayer->GetAmmoCount( pActiveWeapon->GetPrimaryAmmoType() ), clrAmmo, false );
		}
