...
void CHudHealth::Paint()
{
	...
	if (!health_label)
	{
		health_label = gHUD.GetIcon("health_label");
	...
	int nCrossWidth = health_label->Width();

	x = nCrossWidth + (nFontWidth / 2);
	//x = nCrossWidth / 2 + 10;
	y = GetTall() - ( nFontHeight * 2 ) - 1;

	health_label->DrawSelf(x, y, clrHealth);

	x = nCrossWidth + ( nFontWidth / 2 );
	y = GetTall() - (nFontHeight * 1.5);
	x = DrawHudNumber( x, y, m_iHealth, clrHealth, false );
...
//	int iWidth	= nFontWidth / 10;
	...
	clrHealth.SetColor( 255, 225, 225, a  );
