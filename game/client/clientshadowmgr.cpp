...
static ConVar r_worldlight_castshadows("r_worldlight_castshadows", "1",
	FCVAR_CHEAT, "Allow world lights to cast shadows",
	true, 0, true, 1,
	WorldLightCastShadowCallback);
...
static ConVar r_worldlight_shortenfactor("r_worldlight_shortenfactor", "2",
	FCVAR_CHEAT, "Makes shadows cast from local lights shorter");
static ConVar r_worldlight_mincastintensity("r_worldlight_mincastintensity", "0.1",
	FCVAR_CHEAT,
	"Minimum brightness of a light to be classed as shadow casting",
	true, 0, false, 0);

ConVar r_flashlightdepthtexture("r_flashlightdepthtexture", "1");

ConVar di_shadowmapres("di_shadowmapres", "512.0f", FCVAR_ARCHIVE | FCVAR_REPLICATED);
ConVar di_shadowfiltersize("di_shadowfiltersize", "0.5f", FCVAR_ARCHIVE | FCVAR_REPLICATED);
...
bool CClientShadowMgr::Init()
{
	...
	m_nMaxDepthTextureShadows = 16; //bTools ? 4 : 1;	// Just one shadow depth texture in games, more in tools
	...
	if (!m_bDepthTextureActive)
	{
...
#else
		m_DummyColorTexture.InitRenderTarget(m_nDepthTextureResolution,
			m_nDepthTextureResolution,
			RT_SIZE_NO_CHANGE, nullFormat,
			MATERIAL_RT_DEPTH_NONE, false, "_rt_ShadowDummy");
#endif
		...
		for (int i = 0; i < m_nMaxDepthTextureShadows; i++)
		{
			...
			depthTex.InitRenderTarget(m_nDepthTextureResolution,
				m_nDepthTextureResolution,
				RT_SIZE_NO_CHANGE, dstFormat,
				MATERIAL_RT_DEPTH_ONLY, false, strRTName);
#endif
...
void CClientShadowMgr::BuildFlashlight(ClientShadowHandle_t handle)
{
	...
	if (engine->CullBox(mins, maxs))		return;
