...
extern ConVar ins_dynamic_lighting_muzzleflash;

void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	// If we have an attachment, then stick a light on it.
	//if ( ins_dynamic_lighting_muzzleflash.GetBool() )
	//{
...
//	}
	...
	if (ins_dynamic_lighting_muzzleflash.GetBool())
	{
		...
		if (pPlayer->GetWaterLevel() == 3) // underwater muzzleflashes should be replaced with steam bubbles
			return; 

		else
		{

			C_BaseViewModel *vWeapon = pPlayer->GetViewModel();

			int iAttachment;

			iAttachment = vWeapon->LookupAttachment("muzzle_light");

			C_BaseCombatWeapon *pWeapon = pPlayer->GetActiveWeapon();

			int iMFRed = pWeapon->GetMuzzleFlashRed();
			int iMFGreen = pWeapon->GetMuzzleFlashGreen();
			int iMFBlue = pWeapon->GetMuzzleFlashBlue();
			int iMFRadius = pWeapon->GetMuzzleFlashRadius();

			Vector vAttachment, vAng;
			QAngle angles;
	// new		GetAttachment(2, vAttachment, angles);
			GetAttachment(1, vAttachment, angles); // set 1 instead "attachment"
			AngleVectors(angles, &vAng);
			vAttachment += vAng * 2;

			const char *weapon = pWeapon->GetClassname();

//			if (vWeapon->GetAttachment(2, vAttachment, angles))
//			{
				dlight_t *dl = effects->CL_AllocDlight(index);
				dl->origin = vAttachment;
				dl->color.r = iMFRed;
				dl->color.g = iMFGreen;
				dl->color.b = iMFBlue;
				if (strcmp(weapon, "weapon_cguard") == 0)
					// if Guard's gun is firing, make it emit the dlight through the whole animation since 
					// there are charging beams emitting light (realistically speaking)
				{
					DevMsg("Player's active weapon is weapon_cguard; dl->die is 10.0f\n");
					dl->die = gpGlobals->curtime + 10.0f;
				}
				else
				dl->die = gpGlobals->curtime + 0.05f;
				dl->radius = iMFRadius * 4;
				dl->decay = 512.0f;
				...
				el->color.g = iMFGreen;
				el->color.b = iMFBlue;
				el->radius = iMFRadius * 4;
				el->decay = el->radius / 0.5f;
				if (strcmp(weapon, "weapon_cguard") == 0)
					// if Guard's gun is firing, make it emit the elight through the whole animation since 
					// there are charging beams emitting light (realistically speaking)
				{
					DevMsg("Player's active weapon is weapon_cguard; el->die is 10.0f\n");
					dl->die = gpGlobals->curtime + vWeapon->GetAnimTime();
				}
				else
