...
void C_BaseEntity::CreateLightEffects( void )
{
	...
	if (IsEffectActive(EF_HYDRALIGHT))
	{
		//	C_BaseCombatCharacter *pHydra;

		//	int iAttachment;

		//	iAttachment = pHydra->LookupBone( "Hydra.BoneHead11" );
		...
		dl->color.g = 150;
		dl->color.b = 255;
		dl->radius = random->RandomFloat(1500, 1512);
		dl->die = gpGlobals->curtime + 0.001;
	}
	if (IsEffectActive(EF_BATTERYLIGHT))
	{
		...
		dl->color.g = 255;
		dl->color.b = 255;
		dl->radius = random->RandomFloat(50, 75);
		dl->die = gpGlobals->curtime + 0.001;
	}
	if (IsEffectActive(EF_FIRELIGHT))
	{
		dl = effects->CL_AllocDlight(index);
		dl->origin = GetAbsOrigin();
		dl->color.r = 255;
	...
	if (IsEffectActive(EF_LIGHTSTALKLIGHT))
	{
		dl = effects->CL_AllocDlight(index);
		dl = effects->CL_AllocElight(index);
		dl->origin = GetAbsOrigin();

		dl->color.r = m_clrRender->r;
		dl->color.g = m_clrRender->g;
		dl->color.b = m_clrRender->b;

		dl->radius = m_clrRender->a;
		dl->die = gpGlobals->curtime + 0.001;
	}
