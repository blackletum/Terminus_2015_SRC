...
ConVar ins_flare_scale( "ins_flare_scale", "4.0", FCVAR_ARCHIVE | FCVAR_REPLICATED );
ConVar ins_flarelight_cone( "ins_flarelight_cone", "90.0f", FCVAR_ARCHIVE | FCVAR_REPLICATED );
...
BEGIN_DATADESC( CFlare )
	...
	DEFINE_FIELD( alpha, FIELD_FLOAT ),
...
CFlare::CFlare( void )
{
	m_flScale		= ins_flare_scale.GetFloat();
	m_nBounces		= 0;
	m_bFading		= false;
	m_bLight		= true;
	m_bSmoke		= true;
...
void CFlare::Precache( void )
{
	...
	PrecacheScriptSound( "Weapon_FlareGun.Burn" );
  	...
  	UTIL_PrecacheOther( "env_projectedtexture" );
}
...
void CFlare::Spawn( void )
{
	...
	//if ( di_expensive_flares.GetFloat() != 0 || m_spawnflags & SF_FLARE_NO_DLIGHT )
	//{
	//	m_bLight = false;
	//}
...
CFlare *CFlare::Create( Vector vecOrigin, QAngle vecAngles, CBaseEntity *pOwner, float lifetime )
{
	...
	// Don't add it! If you do, it makes NPCs (and most important, zombies) never step on a flare, thus missing all the fun!
...
//-----------------------------------------------------------------------------
// Purpose: handles flare burning process
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: handles damage and target ignition on first impact
//-----------------------------------------------------------------------------
void CFlare::FlareTouch( CBaseEntity *pOther )
{
	...
	CBreakableProp* pOtherProp = dynamic_cast <CBreakableProp*>( pOther );

	//If the flare hit a person or NPC, do damage here.
	if ( pOtherAnim && pOtherAnim->m_takedamage && !pOtherAnim->IsPlayer() )
	{
		...
		pOtherAnim->TakeDamage( CTakeDamageInfo( this, m_pOwner, 10, DMG_BURN ) );

		if( pOtherAnim->Classify() == CLASS_ZOMBIE )
		{
			pOtherAnim->IgniteLifetime( 15.0f );
		}

		else if( pOtherAnim->Classify() == CLASS_HEADCRAB )
		{
			pOtherAnim->IgniteLifetime( 5.0f );
		}

		else if( strcmp( pOtherAnim->GetClassname(), "npc_combine_s" ) == 0 )
		{
			pOtherAnim->IgniteLifetime( 5.0f );
		}
	...
	else if( pOtherProp && pOtherProp->HasInteraction( PROPINTER_FIRE_FLAMMABLE ) )
	{
		pOtherProp->Break( m_pOwner, CTakeDamageInfo( this, m_pOwner, 100, DMG_BURN ) );
	}
	...
	else
	{
		...
		//Scorch decal
		if ( GetAbsVelocity().LengthSqr() > (150*150) )
		{
...
//		{
			...
			CEnvProjectedTexture *pFlareLight = dynamic_cast< CEnvProjectedTexture * >( CreateEntityByName("env_projectedtexture") );
													
			pFlareLight->Teleport( &origin, &angles, NULL );
			pFlareLight->m_bLightWorld = true;
//			pFlareLight->m_LightColor.Init( 255, 0, 0, alpha );
			pFlareLight->m_LinearFloatLightColor.Init(1.0f, 0.0f, 0.0f);
			pFlareLight->m_flLightFOV = ins_flarelight_cone.GetFloat();
			pFlareLight->m_bState = true;
			pFlareLight->m_flAmbient = 0.0f;
			pFlareLight->m_flFarZ = 500.0f;
			pFlareLight->m_flNearZ = 0.1f;
			pFlareLight->m_bCameraSpace = false;
			pFlareLight->SetParent( this );
			pFlareLight->KeyValue( "texturename", "effects/flashlight001" );
			pFlareLight->m_bEnableShadows = true;

			if( alpha < 1 )
			{
				pFlareLight->m_bState = false;
				UTIL_Remove( pFlareLight );
			}
//		}
	}
}
...
//-----------------------------------------------------------------------------
// Purpose: handles damage and ignition done to target on contact with a lying/sticked flare 
//-----------------------------------------------------------------------------
void CFlare::FlareBurnTouch( CBaseEntity *pOther )
{
	if ( pOther && pOther->m_takedamage && ( m_flNextDamage < gpGlobals->curtime ) )
	{
		...
		pOther->TakeDamage( CTakeDamageInfo( this, m_pOwner, burnDamage, (DMG_BURN) ) );
		...
		{
			pOtherAnim->IgniteLifetime( 8.0f );
		}

		else if( Q_strcmp( pOtherAnim->GetClassname(), "npc_stalker" ) )
		{
			pOtherAnim->IgniteLifetime( 10.0f );
		}
