...
ConVar	sk_combine_assassin_health( "sk_combine_assassin_health","150");
...
LINK_ENTITY_TO_CLASS( npc_combine_assassin, CNPC_Assassin );
...
{
	PrecacheModel( "models/femaleassassin.mdl" );
...
void CNPC_Assassin::Spawn( void )
{
	...
	SetModel( "models/femaleassassin.mdl" );
	...
	m_iHealth			= sk_combine_assassin_health.GetFloat();
...
void CNPC_Assassin::FirePistol( int hand )
{
	...
	int bulletType = GetAmmoDef()->Index( "9x19" );
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( (	HasCondition( COND_SEE_ENEMY ) 
				&& HasCondition( COND_ASSASSIN_ENEMY_TARGETTING_ME ) 
				&& random->RandomInt( 0, 32 ) == 0 
