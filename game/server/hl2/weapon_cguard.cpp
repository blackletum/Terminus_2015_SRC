...
public:
	...
	void Explode( float magnitude )
	{
		...
		enginesound->EmitSound( filter, entindex(), CHAN_WEAPON, "weapons/cguard/cguard_fire.wav", 1, ATTN_NORM ); 
...
public:
	...
	void FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecSrc, Vector &vecAiming ); 
...
void CWeaponCGuard::Precache( void )
{
	...
	m_beamIndex = PrecacheModel( "sprites/tp_beam001.vmt" );
...
void CWeaponCGuard::UpdateLasers( void )
{
	...
	for ( int i = 0; i < 4; i++ )
	{
		...
		UTIL_Beam( tr.endpos, ofs, m_beamIndex, 0, 0, 0.2f, 0.1f, 0, 2, 1, 0, 255, 255, 255, 32, 100 );
		
		UTIL_Beam( tr.endpos, ofs, m_haloIndex, 0, 0, 0.2f, 0.1f, 0, 4, 1, 16, 255, 255, 255, 8, 100 );
	}
}
//-----------------------------------------------------------------------------
// Purpose: 
...
void CWeaponCGuard::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecSrc, Vector &vecAiming )
{
	Vector impactPoint = vecSrc + ( vecAiming * MAX_TRACE_LENGTH );

	trace_t	tr;
	UTIL_TraceHull( vecSrc, impactPoint, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), MASK_SHOT, pOperator, COLLISION_GROUP_NONE, &tr );
...
void CWeaponCGuard::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	...
	{ 
		...
		case EVENT_WEAPON_CGUARD_FIRE:
		{
			...
			vecSrc = pOperator->WorldSpaceCenter() + vecAiming * 64;

			FireNPCPrimaryAttack( pOperator, vecSrc, vecAiming );
		}
		break;
		default:
		{
