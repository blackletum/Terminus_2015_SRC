...
END_DATADESC()
//-------------------------------------
static ConVar	sv_hydraLength( "hydra_length", "200", FCVAR_ARCHIVE, "Hydra Length" );
static ConVar	sv_hydraSlack( "hydra_slack", "200", FCVAR_ARCHIVE, "Hydra Slack" );

static ConVar	sv_hydraSegmentLength( "hydra_segment_length", "50", FCVAR_ARCHIVE, "Hydra Slack" );
...
void CNPC_Hydra::Precache()
{
	...
	enginesound->PrecacheSound("npc/hydra/hydra_alert1.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_alert2.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_alert3.wav");
	
	enginesound->PrecacheSound("npc/hydra/hydra_pain1.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_pain2.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_pain3.wav");
	
	enginesound->PrecacheSound("npc/hydra/hydra_sendtentacle1.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_sendtentacle2.wav");
	enginesound->PrecacheSound("npc/hydra/hydra_sendtentacle3.wav");

	BaseClass::Precache();
}
void CNPC_Hydra::Activate( void )
{	
//	AddEffects( EF_HYDRALIGHT );
...
#define HYDRA_ATTN_IDLE (float) 5.0
void CNPC_Hydra::IdleSound ( void )
{
	/*CPASAttenuationFilter filter( this, HYDRA_ATTN_IDLE );
	switch ( random->RandomInt(0,2) )
	{
	case 0:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_sendtentacle1.wav", 1, HYDRA_ATTN_IDLE );	
		break;
	case 1:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_sendtentacle2.wav", 1, HYDRA_ATTN_IDLE );	
		break;
	case 2:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_sendtentacle3.wav", 1, HYDRA_ATTN_IDLE );	
		break;
	}

	m_flNextIdleSoundTime = gpGlobals->curtime + random->RandomFloat( 5, 10 );*/
}
#define HYDRA_ATTN_PAIN (float) 5.0

void CNPC_Hydra::PainSound( const CTakeDamageInfo &info )
{
	/*CPASAttenuationFilter filter( this, HYDRA_ATTN_PAIN );
	switch ( random->RandomInt(0,2) )
	{
	case 0:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_pain1.wav", 1, HYDRA_ATTN_PAIN );	
		break;
	case 1:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_pain2.wav", 1, HYDRA_ATTN_PAIN );	
		break;
	case 2:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_pain3.wav", 1, HYDRA_ATTN_PAIN );	
		break;
	}
	m_flNextPainSoundTime = gpGlobals->curtime + random->RandomFloat( 2, 5 );*/
}
#define HYDRA_ATTN_ALERT (float) 5.0

void CNPC_Hydra::AlertSound( void )
{
	/*CPASAttenuationFilter filter( this, HYDRA_ATTN_ALERT );
	switch ( random->RandomInt(0,2) )
	{
	case 0:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_alert1.wav", 1, HYDRA_ATTN_ALERT );	
		break;
	case 1:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_alert2.wav", 1, HYDRA_ATTN_ALERT );	
		break;
	case 2:	
		enginesound->EmitSound( filter, entindex(), CHAN_BODY, "npc/hydra/hydra_alert3.wav", 1, HYDRA_ATTN_ALERT );	
		break;
	}
	m_flNextAlertSoundTime = gpGlobals->curtime + random->RandomFloat( 5, 10 );*/
}
//-------------------------------------
...
void CNPC_Hydra::Stab( CBaseEntity *pOther, const Vector &vecSpeed, trace_t &tr )
{
	...
	{		
		DevMsg( "Hit!\n" );
		ClearMultiDamage();
		CTakeDamageInfo stabdamage( this, this, 50, DMG_SLASH );
		pOther->DispatchTraceAttack( ( stabdamage ), dir, &tr );
	}
	else if (pOther->m_takedamage == DAMAGE_YES && !pOther->IsPlayer())
	{		
		ClearMultiDamage();
		CTakeDamageInfo info( this, this, 100, DMG_SLASH );
		...
		if ( pAnimating )
		{
			if( Q_strcmp( pAnimating->GetClassname(), "prop_physics") != 0 )
			{
	...
	CPASAttenuationFilter filter( this );
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
	//m_pExtendTentacleSound = controller.SoundCreate( filter, entindex(), "NPC_Hydra.Attack" );
...
void CNPC_Hydra::AdjustLength( )
{
	...
	else if (m_flCurrentLength > m_idealLength)
	{
		...
		else if (gpGlobals->frametime - m_flLastAdjustmentTime > 1.0)
		{
			bShouldAdjust = true;
			// start to panic
			if (!GrowFromMostStretched( ))
