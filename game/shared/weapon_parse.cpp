...
#include "weapon_swapping.h"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"


extern ConVar ins_allow_weapon_swapping;

#define WEAPON_AMMONAME_MISSING "!!! Missing ammo name"
...
FileWeaponInfo_t::FileWeaponInfo_t()
{
	...
	m_bCanDrop = true;
	m_bHideAmmoStock = false;
	m_bHideAmmoSecondary = false;
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	// Ammo name
	Q_strncpy( szAmmoName, pKeyValuesData->GetString("ammoname", WEAPON_AMMONAME_MISSING), MAX_WEAPON_STRING);
	...
	if (ins_allow_weapon_swapping.GetBool())
	{
	...
	{
		iSlot = pKeyValuesData->GetInt("bucket_nolimit", 0);
		iPosition = pKeyValuesData->GetInt("bucket_position_nolimit", 0);
	}
	...
	m_bHideAmmoStock = (pKeyValuesData->GetInt("HideAmmoStock", 0) != 0) ? true : false;
	m_bHideAmmoSecondary = (pKeyValuesData->GetInt("HideAmmoSecondary", 0) != 0) ? true : false;
	...
	// Combine Energy Rounds table
	iCRDamageType = pKeyValuesData->GetInt("combineenergy_damagetype", 0);
	iCRTracer = pKeyValuesData->GetInt("combineenergy_tracertype", 0);
	iCRPlrDmg = pKeyValuesData->GetInt("combineenergy_plr_dmg", 0);
	iCRNpcDmg = pKeyValuesData->GetInt("combineenergy_npc_dmg", 0);
	flCRImpulse = pKeyValuesData->GetFloat("combineenergy_impulse", 0);
	...
	if (pMuzzleFlash)
	{
		iMuzzleFlashRed = pMuzzleFlash->GetInt("muzzleflash_red", 255);
		iMuzzleFlashGreen = pMuzzleFlash->GetInt("muzzleflash_green", 255);
		iMuzzleFlashBlue = pMuzzleFlash->GetInt("muzzleflash_blue", 255);
		iMuzzleFlashRadius = pMuzzleFlash->GetInt("muzzleflash_radius", 0);
	}
