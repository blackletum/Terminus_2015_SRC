...
ConVar	sk_mass_sniper_round("sk_mass_50bmg", "0", FCVAR_REPLICATED);
ConVar	sk_vel_sniper_round("sk_vel_50bmg", "0", FCVAR_REPLICATED);

// Gausss gun ammo
ConVar	sk_plr_dmg_gauss("sk_plr_dmg_gauss", "0", FCVAR_REPLICATED);
ConVar	sk_plr_max_dmg_gauss("sk_npc_dmg_gauss", "0", FCVAR_REPLICATED);
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		//AlyxGun Ammo (retail, probably will use something else)
		def.AddAmmoType("AlyxGun",			DMG_BULLET,					TRACER_LINE,			"sk_plr_dmg_alyxgun",		"sk_npc_dmg_alyxgun",		"sk_max_alyxgun",		BULLET_IMPULSE(200, 1225), 0 );
		//Pistol Ammo (retail)
		def.AddAmmoType("Pistol",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_pistol",		"sk_npc_dmg_pistol",		"sk_max_pistol",		BULLET_IMPULSE(200, 1225), 0 );
		//SMG1 Ammo (retail, will probably use something else)
		def.AddAmmoType("SMG1",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_smg1",			"sk_npc_dmg_smg1",			"sk_max_smg1",			BULLET_IMPULSE(200, 1225), 0 );
		//Magnum ammo (retail)
		def.AddAmmoType("357",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_357",			"sk_npc_dmg_357",			"sk_max_357",			BULLET_IMPULSE(800, 5000), 0 );
		//Crossbow ammo (retail)
		def.AddAmmoType("XBowBolt",			DMG_BULLET,					TRACER_LINE,			"sk_plr_dmg_crossbow",		"sk_npc_dmg_crossbow",		"sk_max_crossbow",		BULLET_IMPULSE(800, 8000), 0 );
		//RPG ammo (retail)
		def.AddAmmoType("RPG_Round",		DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_rpg_round",		"sk_npc_dmg_rpg_round",		"sk_max_rpg_round",		0, 0 );
		//SMG1 Grenade ammo (retail, used by the OICW)
		...
		//Gauss gun ammo used by the jeep
		...
		//Retail AR2 plasma/dark energy shit ball. Used by the combine guard gun
		def.AddAmmoType("AR2AltFire", DMG_DISSOLVE, TRACER_NONE, 0, 0, "sk_max_ar2_altfire", 0, 0);

		/////Mostly custom ammo
		//Ammo system for all the SMG's, MG's and whatever selse
		...
		//Sniper rifle ammo
		def.AddAmmoType("SniperRound",			DMG_BULLET | DMG_SNIPER, TRACER_NONE,			"sk_plr_dmg_sniper_round", "sk_npc_dmg_sniper_round", "sk_max_sniper_round", BULLET_IMPULSE(sk_mass_sniper_round.GetFloat(), sk_vel_sniper_round.GetFloat()), 0);
		//Thumper ammo(?)
		def.AddAmmoType("Thumper",			DMG_SONIC,					TRACER_NONE,			10, 10, 2, 0, 0 );
		//Physgun pellet ammo (not used because our physgun doesn't use the pellets)
		def.AddAmmoType("Gravity",			DMG_CLUB,					TRACER_NONE,			0,	0, 8, 0, 0 );
		//Extinguisher ammo
		def.AddAmmoType("Extinguisher",		DMG_BURN,					TRACER_NONE,			0,	0, 100, 0, 0 );
		//Gauss gun ammo used by the actual gauss gun itself
		def.AddAmmoType("GaussAmmo",			DMG_SHOCK | DMG_ENERGYBEAM, TRACER_NONE, "sk_plr_dmg_gauss", "sk_npc_dmg_gauss", "sk_max_gauss_round", BULLET_IMPULSE(650, 8000), 0);
		// SLAM ammo
		def.AddAmmoType("slam",				DMG_BURN,					TRACER_NONE,			0, 0, 5, 0, 0);
		// Immolator ammo
		def.AddAmmoType("Immolator",		DMG_BURN, TRACER_NONE, 10, 10, 100, 0, 0);
		// Different SMG2 ammo (I have to make this because the SMG2 is supposed to have different damage
//		def.AddAmmoType("SMG2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_smg2",			"sk_npc_dmg_smg2",			"sk_max_smg2",			BULLET_IMPULSE(200, 1225), 0);
		...
		// BrickBat ammo (gun will probably not be used since it's pretty useless.
		def.AddAmmoType("Brickbat", DMG_CRUSH, TRACER_NONE, 10, 10, 1, 0, 0);
